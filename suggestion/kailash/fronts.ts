 ...
 * kailash/kailash realm. On integration, everything here merges into
...
import { kailashStatus, type KailashAct } from '../../src/content/universes/f4-runtime'
 ...
 * selectKailashCycle / setKailashAct / cycleKailashAct). Only
...
export function markKailashCycleEdited(state: NumericLawState): void {
  if (readNumber(state, 'kailash-front-phase', PHASE_CALM, PHASE_ACTIVE) !== PHASE_CALM) {
...
export function kailashFrontStatus(
  ...
  const phaseIndex = Math.floor(readNumber(state, 'kailash-front-phase', PHASE_CALM, PHASE_ACTIVE))
  const phase: KailashFrontPhase = phaseIndex === PHASE_ACTIVE ? 'active' : phaseIndex === PHASE_APPROACHING ? 'approaching' : 'calm'
  const elapsed = readNumber(state, 'kailash-front-elapsed', 0, phaseDuration(phaseIndex))
...
export function advanceKailashFronts(
  ...
  if (remaining > fullCycle * 2) {
    ...
    writeNumber(state, 'kailash-front-index', (Math.floor(readNumber(state, 'kailash-front-index', 0, KAILASH_FRONTS.length - 1)) + 1) % KAILASH_FRONTS.length)
    writeNumber(state, 'kailash-front-phase', PHASE_CALM)
  ...
  while (remaining > 0) {
    const phase = Math.floor(readNumber(state, 'kailash-front-phase', PHASE_CALM, PHASE_ACTIVE))
    const elapsed = readNumber(state, 'kailash-front-elapsed', 0, phaseDuration(phase))
    ...
    writeNumber(state, 'kailash-front-elapsed', 0)
    if (phase === PHASE_CALM) {
      writeNumber(state, 'kailash-front-phase', PHASE_APPROACHING)
      ...
      writeNumber(state, 'kailash-front-phase', PHASE_ACTIVE)
 ...
 * multiplies this into f4RateMultiplier('kailash', ...) — it is the ONLY
 ...
 * existing kailashStatus().multiplier inside f4RateMultiplier on integration.
 */
