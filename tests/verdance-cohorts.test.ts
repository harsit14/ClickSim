...
test('Verdance cohorts age through four deterministic non-withering stages', () => {
  assert.equal(verdanceStageForAge(0), 'verdance-cohort-new')
  assert.equal(verdanceStageForAge(5 * 60_000), 'verdance-cohort-rooted')
  assert.equal(verdanceStageForAge(60 * 60_000), 'verdance-cohort-mature')
  assert.equal(verdanceStageForAge(8 * 60 * 60_000), 'verdance-cohort-ancient')
  assert.equal(advanceVerdanceCohorts([plantVerdanceCohort(4)], 10 * 60 * 60_000)[0].stageId, 'verdance-cohort-ancient')
})
...
test('offline aging respects the caller cap and compacts by stage', () => {
  ...
  assert.deepEqual(rows, [{ stageId: 'verdance-cohort-rooted', quantity: 5, ageMs: 5 * 60_000 }])
  assert.equal(compactVerdanceCohorts(rows).length, 1)
})

test('production and Pruning reward maturity instead of raw count', () => {
  const rows = [
    { stageId: 'verdance-cohort-new' as const, quantity: 5, ageMs: 0 },
    { stageId: 'verdance-cohort-mature' as const, quantity: 5, ageMs: 60 * 60_000 },
    { stageId: 'verdance-cohort-ancient' as const, quantity: 5, ageMs: 8 * 60 * 60_000 },
  ]
