...
test('phase one exposes plain labels, contextual trial actions, and readable ceremony chrome', () => {
  ...
  assert.match(deep, /function beginTrial\(id: string\)[\s\S]*startChallenge\(id\)[\s\S]*clearBuffs\(\)[\s\S]*onclose\(\)/)
...
test('phase one Garden questions are persistently readable and keyboard-selectable', () => {
  ...
  assert.match(garden, /function readPresence\(universeId: UniverseId\)[\s\S]*selectedLinkId = null[\s\S]*selectedUniverseId = universeId/)
  assert.match(garden, /onfocus=\{\(\) => readPresence\(node\.universeId\)\}/)
  assert.match(garden, /<nav class="relation-names" aria-label="Relations among the restored worlds and lokas">/)
  assert.match(garden, /\.relation-names button \{[^}]*\.6875rem/)
...
test('phase three routes Deep and Remembrance through one structured comparison and authored ceremony', () => {
  ...
  assert.match(deep, />\{identity\.collapseName\} &nbsp;·&nbsp; gain/)
  assert.doesNotMatch(deep, /Review \{identity\.collapseName\}/)
