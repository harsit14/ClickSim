...
test('Verdance Growth Rings use a contained card map and compile accessibly', () => {
  const compiled = compile(observatorySource, {
    filename: observatoryPath.pathname,
    generate: 'client',
  })
  assert.deepEqual(compiled.warnings, [])
  assert.match(observatorySource, /\{#if verdance\}[\s\S]*class="growth-map"/)
  assert.match(observatorySource, /class="growth-node crown \{crownState\}"/)
  assert.match(observatorySource, /class="growth-node \{state\}"/)
})

test('the four Growth Ring paths and capstone cover every persistent node exactly once', () => {
