...
test('narrow CSS uses the same single stacking order as the collision planner', () => {
  ...
  assert.match(app, /id="mobile-more-button"[\s\S]*aria-controls="dock-sections"[\s\S]*aria-expanded=\{shell\.sectionsOpen\}/)
  assert.match(app, /id="dock-sections"[\s\S]*aria-modal=\{shell\.sectionsOpen \? 'true' : undefined\}/)
  ...
  assert.match(app, /class:shop-collapsed=\{shell\.shopCollapsed\}/)
  assert.match(app, /class:shop-absent=\{!hasUi\('shop'\)\}/)
  assert.match(app, /createShellState\(window\.innerWidth, MOBILE_BREAKPOINT\)/)
  assert.match(app, /id="mobile-kindling-button"[\s\S]*aria-controls="kindling-shop"[\s\S]*aria-expanded=\{!shell\.shopCollapsed\}/)
