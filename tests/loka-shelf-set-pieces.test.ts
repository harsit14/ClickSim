...
test('nine authored shelf moments cover exactly three shelves in every loka', () => {
  ...
  for (const universeId of ['brahmalok', 'vishnulok', 'kailash'] as const) {
...
test('a shelf moment lasts twelve seconds, is skippable by persisted timestamp, and has reduced-motion equivalence', () => {
  ...
  const active = planLokaShelfSetPieces('brahmalok', archiveIds('brahmalok', 4), { 'brahmalok-shelf-1-at': startedAt }, startedAt + 3_000, true)[0]
  assert.equal(active.completed, true)
  assert.equal(active.active, true)
  assert.equal(active.reducedMotion, true)
  assert.equal(active.progressFraction, 0.25)
  assert.equal(active.progressKey, 'brahmalok-shelf-1-at')

  const finished = planLokaShelfSetPieces('brahmalok', archiveIds('brahmalok', 4), { 'brahmalok-shelf-1-at': startedAt }, startedAt + LOKA_SHELF_MOMENT_DURATION_MS, false)[0]
  assert.equal(finished.active, false)
  const skipped = planLokaShelfSetPieces('brahmalok', archiveIds('brahmalok', 4), { 'brahmalok-shelf-1-at': 1 }, startedAt, false)[0]
  assert.equal(skipped.active, false)
})

test('completed shelves retain their world landmark after the timed moment', () => {
  const plans = planLokaShelfSetPieces('kailash', archiveIds('kailash', 12), {}, Date.now(), false)
