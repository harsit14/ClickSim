...
import { kailashOwnershipThreshold, planKailashFormations } from '../src/render/canticle/world-layer'
...
test('Kailash endgame uses six capped three-Kindling landforms outside the Heart clearing', () => {
  const owned = Object.fromEntries(CANTICLE_V2_PACK.visual.objects
    .filter(({ sourceKind }) => sourceKind === 'generator')
    .map(({ sourceId }) => [sourceId, 125]))
  const high = planKailashFormations(CANTICLE_V2_PACK.visual.objects, owned, 'high')
  const low = planKailashFormations(CANTICLE_V2_PACK.visual.objects, owned, 'low')
...
test('Kailash preserves the visible Heart above an apertured summit', () => {
  ...
  assert.match(world, /\.canticle \.heart-frame \{/)
