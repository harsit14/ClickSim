...
export const CABINET_RESONANCE_PER_ITEM = 0.01
...
export function curiosityProductionMult(held: readonly string[], cabinet = EMBERLIGHT_CABINET): number {
  ...
  const shelfMultiplier = cabinet.shelves
    .filter((shelf) => shelf.rewardKind === 'production' && curiosityShelfComplete(held, shelf.id, cabinet))
    .reduce((multiplier, shelf) => multiplier * shelf.rewardValue, 1)
  return resonance * shelfMultiplier
}
...
export const TIDEFALL_CURIOSITY_SHELVES: CuriosityShelfDef[] = [
  ...
  { id: 'pilgrims', index: 'II', name: 'The Pelagic Signals', lore: 'Messages carried by migrations, impossible beams, and water moving inward.', ids: ['second-cursor', 'snail', 'aurora', 'door'], rewardKind: 'production', rewardName: 'Living Current', reward: 'all production ×1.15', rewardValue: 1.15 },
