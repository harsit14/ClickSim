...
function generatorObject(index: number): WorldObjectManifest {
  ...
  const objectId = `u3-object-kindling-${String(index + 1).padStart(2, '0')}`
...
export const VERDANCE_ARCHIVE_OBJECTS: WorldObjectManifest[] = VERDANCE_CURIOSITIES.map((item, index) => {
  ...
  return {
    id: `u3-object-archive-${String(index + 1).padStart(2, '0')}`,
    sourceKind: 'archive',
    sourceId: item.id,
    ...
    audioCue: 'u3-archive-reveal',
    loreRecord: item.id,
  }
})

const OMEN_DATA = [
  ['u3-omen-golden-pollinator', 'Golden Pollinator', 'a four-winged pollinator tracing a phyllotaxis route', 'Follow its flower route to multiply Sap.'],
  ['u3-omen-spore-rain', 'Spore Rain', 'a slanting veil of spores rooting wherever it settles', 'Plants young cohorts and accelerates root connections.'],
  ['u3-omen-sunbreak', 'Sunbreak', 'a narrow moving shaft of light crossing the canopy', 'Empowers each Kindling crossed by the light.'],
...
export const VERDANCE_ARCHIVE: ArchiveDef = {
  ...
  shelves: [
    { id: 'u3-shelf-survival', name: 'Survival', recordIds: VERDANCE_CURIOSITIES.slice(0, 4).map(({ id }) => id) as unknown as FourTuple<string>, rewardDescription: 'Offline aging keeps full speed to the declared cap.' },
    { id: 'u3-shelf-communication', name: 'Communication', recordIds: VERDANCE_CURIOSITIES.slice(4, 8).map(({ id }) => id) as unknown as FourTuple<string>, rewardDescription: 'Grafts transmit one visible trait farther through the root network.' },
    { id: 'u3-shelf-inheritance', name: 'Inheritance', recordIds: VERDANCE_CURIOSITIES.slice(8, 12).map(({ id }) => id) as unknown as FourTuple<string>, rewardDescription: 'Mature and ancient cohorts yield stronger Memory Seed inheritance.' },
  ],
}
...
export const VERDANCE_AUDIO: UniverseAudioDef = {
  ...
  cues: [
    cue('u3-click-seedcoat', 'touch', 'seedcoat-and-root', 'normal', 'none', 80),
    cue('u3-purchase-root', 'purchase', 'wooden-sixth-rise', 'normal', 'visual', 120),
    cue('u3-critical-bloom', 'touch', 'flowering-accent', 'important', 'visual', 180),
    cue('u3-omen-call', 'omen', 'pollinator-and-leaf-call', 'important', 'visual-and-caption', 500),
    cue('u3-pruning-cadence', 'ceremony', 'tender-pruning-cadence', 'ceremony', 'visual-and-caption', 2_500),
    cue('u3-archive-reveal', 'story', 'herbarium-page-and-breath', 'important', 'caption', 500),
    cue('u3-beacon-flower', 'ceremony', 'world-tree-beacon-flower', 'ceremony', 'visual-and-caption', 2_500),
  ],
  clickMaterialCue: 'u3-click-seedcoat',
  purchaseIntervalCue: 'u3-purchase-root',
  criticalAccentCue: 'u3-critical-bloom',
  omenCallCue: 'u3-omen-call',
  prestigeCadenceCue: 'u3-pruning-cadence',
  stems: [
    { id: 'u3-stem-roots', kindlingFamily: 'u3-kindling-01', bus: 'music', description: 'Root-bass pulses and seed-shell percussion.' },
    { id: 'u3-stem-groves', kindlingFamily: 'u3-kindling-06', bus: 'music', description: 'Wooden hand percussion and leaf-noise rhythm.' },
    { id: 'u3-stem-pollinators', kindlingFamily: 'u3-kindling-07', bus: 'music', description: 'Breathy flute synthesis following pollination routes.' },
    { id: 'u3-stem-world-root', kindlingFamily: 'u3-kindling-16', bus: 'music', description: 'Deep vascular pulse and canopy harmony.' },
  ],
...
const VERDANCE_BEACON_OBJECT: WorldObjectManifest = {
  id: 'u3-object-beacon-world-tree',
  sourceKind: 'beacon',
  sourceId: 'u3-beacon-world-tree',
  ...
  audioCue: 'u3-beacon-flower',
  loreRecord: 'u3-echo-10',
}
...
export const VERDANCE_V2_SUPPLEMENT: UniversePackV2Supplement = {
  ...
  economy: {
    ...
    localPrestige: {
      ...
      rewardCurrency: { id: 'u3-memory-seeds', canonicalName: 'Epoch Matter', localName: 'Memory Seeds', singular: 'Memory Seed', plural: 'Memory Seeds', glyph: '◇', material: 'living seed shells inscribed with mature growth rings', scope: 'epoch' },
      gainFormulaId: 'u3-pruning-maturity-gain',
      ...
      ceremonyFeedbackId: 'u3-pruning-cadence',
    },
  },
  heart: {
    id: 'u3-first-seed', canonicalName: 'Heart', localName: 'First Seed',
  ...
  story: {
    civilizationQuestion: 'Is preservation still life if nothing is allowed to change?',
    scenes: [
      { id: 'u3-scene-arrival', kind: 'arrival', skippableAfterFirstView: true, replayable: true },
      { id: 'u3-scene-pruning', kind: 'epoch', skippableAfterFirstView: true, replayable: true },
      { id: 'u3-scene-deep-roots', kind: 'deep', skippableAfterFirstView: true, replayable: true },
      { id: 'u3-scene-world-tree-beacon', kind: 'beacon', skippableAfterFirstView: true, replayable: true },
      { id: 'u3-scene-amber-transmission', kind: 'transmission', skippableAfterFirstView: true, replayable: true },
    ],
  },
  ...
  beacon: {
    id: 'u3-beacon-world-tree', canonicalName: 'Beacon', localName: 'The Garden Flower',
    requirement: { sourceKind: 'generator', sourceId: 'u3-kindling-18', target: 1, description: 'Grow one World-Tree capable of opening beyond its native sky.' },
    darkBetweenReward: 3,
  ...
  accessibility: {
    ...
    screenReaderOrder: ['u3-first-seed', ...VERDANCE_KINDLING_OBJECTS.map(({ id }) => id), 'u3-impossible-herbarium', 'u3-beacon-world-tree'],
    announcements: [
    ...
    nonColorSignals: [
      { stateId: 'u3-cohort-new', text: 'New cohort', shape: 'closed seed', pattern: 'one ring', highContrastTreatment: 'solid white seed outline and NEW label' },
      { stateId: 'u3-cohort-rooted', text: 'Rooted cohort', shape: 'seed with one root fork', pattern: 'two rings', highContrastTreatment: 'white root fork and ROOTED label' },
      { stateId: 'u3-cohort-mature', text: 'Mature cohort', shape: 'open canopy', pattern: 'three rings', highContrastTreatment: 'open crown and MATURE label' },
      { stateId: 'u3-cohort-ancient', text: 'Ancient cohort', shape: 'broad crown and root ring', pattern: 'four rings', highContrastTreatment: 'double crown, root ring, and ANCIENT label' },
    ],
