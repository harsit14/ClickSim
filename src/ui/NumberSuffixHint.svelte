...
<style>
  ...
  .suffix-hint > span { color:var(--amber);font:760 .42rem/1 system-ui,sans-serif;letter-spacing:.13em; }
  .suffix-hint p { grid-column:1 / -1;margin:0;color:color-mix(in srgb,var(--gold) 78%,white);font:560 .62rem/1.25 system-ui,sans-serif; }
  .suffix-hint p strong { color:white;font:780 .72rem/1 ui-monospace,monospace; }
  .suffix-hint sup { font-size:.42rem; }
  .suffix-hint small { grid-column:1;color:var(--dim);font:.46rem/1.25 system-ui,sans-serif; }
  .suffix-hint button { grid-column:2;grid-row:1;padding:0;color:var(--dim);background:transparent;border:0;font:680 .43rem/1 system-ui,sans-serif;text-transform:uppercase;cursor:pointer; }
