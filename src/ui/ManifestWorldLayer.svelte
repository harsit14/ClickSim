  ...
  {#if presentation}
    ...
    <div class="scene-composition" aria-hidden="true">
      ...
      {:else if pack.id === 'kailash'}
        ...
        <svg class="kailash-wind-line" viewBox="0 0 900 120" preserveAspectRatio="none">
          <path d="M8 74C128 18 208 96 326 54C438 14 522 86 640 48C742 15 811 31 892 12"></path>
        </svg>
...
<style>
  ...
  .brahmalok-lotus span { position: absolute; inset: 42%; border: 1px dashed color-mix(in srgb, #8ecbe0 48%, transparent); background: transparent; transform: rotate(45deg); }
  ...
  .kailash-wind-line { position:absolute;z-index:2;left:8%;top:27%;width:72%;height:15%;overflow:visible;opacity:.38;filter:drop-shadow(0 0 .35rem color-mix(in srgb,#c8dde6 14%,transparent)); }
  .kailash-wind-line path { vector-effect:non-scaling-stroke;fill:none;stroke:color-mix(in srgb,#d6e4e9 34%,transparent);stroke-width:1.2;stroke-linecap:round;stroke-dasharray:44 12 7 18;animation:wind-carry 18s ease-in-out infinite alternate; }
  .kailash-snow { position:absolute;z-index:3;inset:7% 2% 20%;overflow:hidden;pointer-events:none; }.kailash-snow i { position:absolute;left:var(--snow-x);top:var(--snow-y);width:calc(.14rem * var(--snow-size));aspect-ratio:1;border-radius:50%;background:color-mix(in srgb,#e4eef2 58%,transparent);box-shadow:0 0 .3rem color-mix(in srgb,#bfd9e3 22%,transparent);opacity:calc(.38 + var(--snow-size) * .25);animation:snow-drift calc(8s + var(--snow-index) * .28s) ease-in-out infinite alternate;animation-delay:calc(var(--snow-index) * -.73s); }
  ...
  @keyframes wind-carry { to { transform:translateX(2.2rem);stroke-dashoffset:-48;opacity:.58; } }
  ...
  .motion-paused .kailash-wind-line path { animation:none; }
  ...
  .low-quality .kailash-wind-line { opacity:.24; }
